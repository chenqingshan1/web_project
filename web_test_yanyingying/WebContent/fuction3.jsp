...
//	System.out.println(date);
	...
	String url=null;
     try {  
    	 if (env.equals("kaifa")) {
    		 url = "http://192.168.1.166:28080/block/payState/reset/endDate";  
             HttpClient client = new HttpClient();  
             PostMethod postMethod = new PostMethod(url);  
             postMethod.addParameter("jid",jid);
             postMethod.addParameter("date",date); 
             responseCode = client.executeMethod(postMethod); 
 		} else if (env.equals("qa")) {
 			  url = "http://192.168.1.22:28080/block/payState/reset/endDate";  
             HttpClient client = new HttpClient();  
             PostMethod postMethod = new PostMethod(url);  
             postMethod.addParameter("jid",jid);
             postMethod.addParameter("date",date); 
